...
> **项目定位**: 跨平台聊天机器人三轴记忆系统
> **技术栈**: Python 3.10+, AstrBot 4.0+, SQLite + FTS5, FAISS, aiosqlite
> **文档版本**: v0.1（初始规划版）
> **生成时间**: 2026-05-02
> **项目状态**: 设计完成，待实现
...
| 全文检索 | Tantivy | 无 | SQLite FTS5（零依赖） |
...
[领域模型层]     ← 纯 Python dataclass，零 I/O
  Persona | Event | Impression
       │
       ▼
[仓储层]         ← 抽象接口，生产用 SQLite 实现
  ├─► SQLite + FTS5  （结构化 + 关键词搜索）
  ├─► FAISS          （向量语义搜索）
...
├── db/
│   ├── core.db          # SQLite WAL 模式：events, personas, impressions, fts5
...
**验收标准**：`pytest tests/` 全绿，无任何 SQLite/文件/网络 I/O

---

### Phase 2 — SQLite 仓储实现 + Schema 迁移
**目标**：生产级持久化，aiosqlite，WAL 模式

**交付文件**：
```
repository/
└── sqlite.py        # SQLitePersonaRepo, SQLiteEventRepo, SQLiteImpressionRepo
migrations/
├── 001_initial_schema.sql
└── runner.py        # 迁移运行器（无第三方依赖）
tests/
└── test_sqlite_repo.py  # 集成测试（tmpdir DB）
```

**验收标准**：内存实现的测试用例可在 SQLite 实现上全部通过（接口置换测试）
...
| 亮点 | 实现方式 | 价值 |
|---|---|---|
| **接口置换测试** | 内存实现 + SQLite 实现共用一套测试 | 早期发现接口语义不一致 |
