...
import { ArrowRightLeft, Bot, ChevronsUpDown, Globe, LogOut } from 'lucide-react'
...
/**
 * Sidebar footer widget that combines persona switching and logout into one
 * compact trigger: Avatar + current persona name → Popover.
 *
 * Rendered at the very bottom of AppSidebar's SidebarFooter.
 * The persona-selector section that was previously in SidebarContent is removed
 * in favour of this component.
 */
export function SidebarUserMenu() {
  const { i18n, currentPersonaName, scopeMode, setCurrentPersona, personaConfig, authEnabled, toast } = useApp()
  ...
  const visibleBots = useMemo(() => {
    if (!personaConfig.isolationEnabled) return []
    if (isAll || bots.some(b => b.name === currentPersonaName)) return bots
    return [...bots, { name: currentPersonaName, event_count: 0 }]
  }, [bots, currentPersonaName, isAll, personaConfig.isolationEnabled])
  ...
  const showPersonaSection = personaConfig.isolationEnabled
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          <span className="group-data-[collapsible=icon]:hidden flex-1 truncate text-left text-xs font-medium">
            {label}
          </span>
          <ChevronsUpDown className="group-data-[collapsible=icon]:hidden h-3 w-3 shrink-0 text-muted-foreground/50" />
        </Button>
      </PopoverTrigger>
      ...
      <PopoverContent side="top" align="start" className="w-56 p-1">
        ...
        {showPersonaSection && (
          <>
            ...
            {authEnabled && <Separator className="my-1" />}
          </>
        )}
        ...
        {authEnabled && (
          <button
            ...
            className="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-sm hover:bg-destructive/10 hover:text-destructive transition-colors text-muted-foreground"
          >
            <LogOut className="h-4 w-4 shrink-0" />
            <span className="flex-1 text-left">{i18n.auth.logout}</span>
          </button>
        )}
      </PopoverContent>
    </Popover>
