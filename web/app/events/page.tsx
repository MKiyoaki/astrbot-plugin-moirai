...
import { Plus, RefreshCw, Trash2, List, Activity, CircleMinus } from 'lucide-react'
...
export default function EventsPage() {
  ...
  useEffect(() => {
    loadEvents()
    api.tags.list().then(r => setTagSuggestions(r.tags.map(t => t.name)))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  // When list view selected, navigate to library/events
  const handleViewChange = (v: View) => {
    if (v === 'list') {
      router.push('/library?tab=events')
      return
    }
    setView(v)
  }
  ...
  const actions = (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <div className="bg-muted flex rounded-lg p-0.5">
        ...
        >
          <Activity className="mr-1 size-3.5" />{i18n.events.timeline}
        </Button>
        <Button
          variant="ghost"
          size="sm"
        ...
        >
          <List className="mr-1 size-3.5" />{i18n.events.list}
        </Button>
      </div>
