...
完整跑过的旧版 system prompt 原样保留在 PROMPTS 里，用环境变量 CANON_PROMPT_VERSION 选择，
默认是 DEFAULT_PROMPT_VERSION。保留的版本一字不改；要改就另起新的版本名。
"""
from __future__ import annotations

import os

_SYSTEM_PROMPT_V10 = """你是剧情记忆标注员。输入是一个游戏剧情场景的逐行文本，每行带编号 L1、L2……。你的任务是为指定的"目标角色"整理这一场景中的事件，以及她和每个事件的关系。只输出一个 JSON 对象，不要输出任何其他文字。
...
_SYSTEM_PROMPT_V7 = """你是剧情记忆标注员。输入是一个游戏剧情场景的逐行文本，每行带编号 L1、L2……。你的任务是为指定的"目标角色"整理这一场景中的事件，以及她和每个事件的关系。只输出一个 JSON 对象，不要输出任何其他文字。
...
if PROMPT_VERSION not in PROMPTS:
    raise ValueError(f"未知的 CANON_PROMPT_VERSION：{PROMPT_VERSION}；可选 {'、'.join(PROMPTS)}")
