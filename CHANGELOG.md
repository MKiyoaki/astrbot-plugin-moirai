...
### 统计页面 UI 细节修复 (Stats Page UI Fixes)

- **响应时长显示优化**：修复了在无性能数据时平均响应时间显示为 `—s` 的异常（现显示为 `—`），提升了界面初始化时的视觉美感。
- **卡片布局对齐**：为“近 30 日活跃趋势”卡片增加了 `h-full` 属性，确保其与“平均指标 (每事件)”卡片在网格布局中保持高度统一，解决了布局不一致的问题。
- **更新前端landing page, stat page和side bar设计语言**。
