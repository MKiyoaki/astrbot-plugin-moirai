...
- 在左下角 Bot 视角切换弹层标题右侧新增常驻设置按钮
- 将跳转按钮文案调整为“我希望迁移人格数据”，并改为更贴合迁移语义的双向箭头样式
- 点击后跳转到配置页的 Persona 数据归属管理区块，并将区块滚动到视口居中
...
- 左下角 Bot 切换器打开时刷新 persona 列表，并在接口短暂漏报时保留当前已选 persona 作为兜底项
- 重新执行前端生产构建并同步 `pages/moirai/_app`，确保 AstrBot 安装场景加载到最新 Persona 迁移工具

**API**

- `/api/personas/bots` 改为从 `events`、`impressions`、`personas` 三处合并枚举 `bot_persona_name`
- 对只存在关系 / 印象或人格标签、暂时没有事件计数的 Persona 返回 `event_count: 0`

**I18n**

- 补齐中文、日文、英文的跳转按钮文案与可访问名称

**Build**

- 修复 Windows 环境下构建同步脚本无法启动 `npm.cmd` 的问题
- 构建脚本读取 npm 输出时改为 UTF-8 容错解码，避免非 GBK 字符导致脚本崩溃

**Version**

- 更新版本到 `v0.12.2`
- 同步更新 `README.md` 与 `README_EN.md` 版本徽章
- `tools/bump_version.py` 新增 README / README_EN 默认版本徽章更新
...
- 在「插件配置 → 社会关系与印象」卡片内部新增 Persona 数据归属管理入口
