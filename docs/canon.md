...
1. 理解（本地）：用实体别名表和几组指代词判断本轮属于闲聊、剧情还是近况。一句话提到多个人物时，按出现顺序最多取 3 人，各自检索。追问省略人名时，只在出现“她/他/那件事”或“上次/当时”等往事指代时沿用用户先前提到的第一个人名，不把模型先前的回答当成检索依据。
...
4. 生成：默认把 `canon_recall` 和 `operator_archive`（见下面的“档案查询”）作为原生 tool 提供给模型（KCL arc:nexus 实测返回结构化 `tool_calls`，寒暄时不调用），模型觉得需要回忆时自行检索，每轮最多两次工具往返，工具往返不写入历史。`--no-tools` 可关闭；接口拒绝 `tools` 时本次会话自动改为不带工具。
